    ...
    private Mono<Map<String, Object>> handleResponse(ClientResponse response) {
        ...
        if (response.statusCode().is3xxRedirection()) {
            ...
            return webClient.post()
                    ...
                    .contentType(MediaType.APPLICATION_JSON)
                    .exchange()
                    .flatMap(this::handleNonRedirectedResponse);
        }
        
        // 处理非重定向响应
        return handleNonRedirectedResponse(response);
    }
    
    /**
     * 处理非重定向的HTTP响应
     */
    private Mono<Map<String, Object>> handleNonRedirectedResponse(ClientResponse response) {
        // 检查响应状态是否成功
        if (response.statusCode().is2xxSuccessful()) {
            // 检查内容类型
            String contentType = response.headers().contentType().map(MediaType::toString).orElse("");
            log.debug("响应内容类型: {}", contentType);
            ...
            if (contentType.contains("application/json")) {
                ...
                return response.bodyToMono(String.class)
                        .map(body -> {
                            log.warn("接收到非JSON响应: {}", body);
                            Map<String, Object> result = new HashMap<>();
                            result.put("raw_response", body);
                            result.put("warning", "Response is not in JSON format");
                            return result;
                        });
            }
        } else {
            ...
            return response.bodyToMono(String.class)
                    ...
                    .flatMap(errorBody -> {
                        log.error("请求失败，状态码: {}, 错误信息: {}", response.statusCode(), errorBody);
                        return Mono.error(new RuntimeException("API请求失败: " + response.statusCode() + ", " + errorBody));
                    });
        }
    }
